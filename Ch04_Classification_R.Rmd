...
Q1. What metric will you use to evaluate covid model? Why? 
...
Your code:
```{r,echo=TRUE}
glm_fit<- glm(SURVIVED_bin~SEX*AGE_TRUNK + PNEUMONIA + TOBACCO*SEX + OTHER_DISEASE,family=binomial,data= Train_COVID)
glm_fit$converged
arm::binnedplot(predict(glm_fit,type="response"),resid(glm_fit,type="response" ))
logit_P = predict(glm_fit,Covid_Data,type="response")
roc_score=roc(Covid_Data$SURVIVED_bin, logit_P) #AUC score
...
~~~
Why you add interaction terms, why you chose certain variables.
...
#(b)
pairs(Auto[,-9])
Auto %>% ggplot(aes(mpg01, horsepower, group = mpg01)) + 
  geom_boxplot() + 
  theme_bw()
Auto %>% ggplot(aes(mpg01, weight, group = mpg01)) + 
  geom_boxplot() + 
  theme_bw()
Auto %>% ggplot(aes(mpg01, displacement, group = mpg01)) + 
  geom_boxplot() + 
  theme_bw()
...
~~~
From the scatterplot matrix, it appears that `horsepower`, `weight`, and `displacement` may be associated with `mpg01`. I then created a boxplot of these variables against `mpg01` and we can see that high or low gas mileage is associated with different levels of horsepower, weight, and displacement suggesting these features may be useful in predicting `mpg01`. 
